...
const _ = db.command
const CB = db.collection('UpDown').doc('CaminoBakery')
...
function initChart(canvas, width, height, dpr) {  
  ...
  var weekdays= {    
    ...
    series: [{name: 'Pit', type: 'bar', label: {normal: {show: false, position: 'inside', color: 'white'}},itemStyle: {borderRadius: [4, 4, 0, 0], color: '#d6b160', shadowColor: 'rgba(0, 0, 0, 0.5)', shadowBlur: 2},  //Series设置
        ...
        data: [5, 25, 35, 40, 30, 15, 10, 10, 12, 10, 16, 18, 15, 5, 5], },]}
  ...
  var weekend= {  
    ...
    series: [{name: 'Pit', type: 'bar', label: {normal: {show: false, position: 'inside', color: 'white'}},itemStyle: {borderRadius: [4, 4, 0, 0], color: '#d6b160', shadowColor: 'rgba(0, 0, 0, 0.5)', shadowBlur: 2},  //Series设置
        // 👇 数据录入处 ！！暂定以10为scale进行模拟, 数据源: Google
        data: [0, 0, 10, 30, 20, 10, 5, 5, 10, 7, 0, 0, 0, 0, 0], },]}
...
Page({
    data: {
        ...
        timeTable:[{realTimeTable:'Mon: 7:30 - 24:00'},{realTimeTable:'Tue: 7:30 - 24:00'},{realTimeTable:'Wed: 7:30 - 24:00'},{realTimeTable:'Thu: 7:30 - 24:00'},{realTimeTable:'Fri: 7:30 - 17:00'},{realTimeTable:'Sat: 12:00 - 17:00'},{realTimeTable:'Sun: 11:00 - 12:00'}],
    },
  ...
  zan: function (item_id) {
    ...
    remark(e){
      ...
      wx.cloud.database().collection('comments').doc('CaminoBakery')
      .update({
      data:{
    ...
    delete: function(e) {
      ...
      // 同步云端删除
      db.collection('comments').doc('CaminoBakery').update({
        data: {
          commentList: list
        }
      })
    ...
    isYourComments: function (e) {
      ...
      wx.cloud.database().collection('comments').doc('CaminoBakery').get().then(res=>{
        this.setData({
