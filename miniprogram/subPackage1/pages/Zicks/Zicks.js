...
const ZK = db.collection('UpDown').doc('Zicks')
...
function initChart(canvas, width, height, dpr) {  
  ...
  var weekdays= {   
    ...
    series: [{name: 'Pit', type: 'bar', label: {normal: {show: false, position: 'inside', color: 'white'}},itemStyle: {borderRadius: [4, 4, 0, 0], color: '#d6b160', shadowColor: 'rgba(0, 0, 0, 0.5)', shadowBlur: 2},  //Series设置
        // 👇 数据录入处 ！！暂定以100为scale进行模拟, 数据源: Google
        data: [0, 0, 0, 11, 45, 43, 28, 22, 27, 33, 55, 65, 50, 33, 23], },]}
  ...
  var weekend= {  
    ...
    series: [{name: 'Pit', type: 'bar', label: {normal: {show: false, position: 'inside', color: 'white'}},itemStyle: {borderRadius: [4, 4, 0, 0], color: '#d6b160', shadowColor: 'rgba(0, 0, 0, 0.5)', shadowBlur: 2},  //Series设置
        // 👇 数据录入处 ！！暂定以100为scale进行模拟, 数据源: Google
        data: [0, 0, 0, 10, 40, 38, 25, 20, 20, 34, 50, 60, 45, 40, 35], },]}
...
// ************** 评论功能所需onLoad结束*************//
   ...
   // 点赞函数
    ...
    remark(e){
      ...
      wx.cloud.database().collection('comments').doc('Zicks')
      .update({
      data:{
    ...
    delete: function(e) {
      ...
      // 同步云端删除
      db.collection('comments').doc('Zicks').update({
        data: {
          commentList: list
        }
      })
    ...
    isYourComments: function (e) {
      ...
      wx.cloud.database().collection('comments').doc('Zicks').get().then(res=>{
        this.setData({
