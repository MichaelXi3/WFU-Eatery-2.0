...
const _ = db.command
const ST = db.collection('UpDown').doc('Shorty')
...
function initChart(canvas, width, height, dpr) {  
  ...
  var weekdays= {    
    ...
    series: [{name: 'Pit', type: 'bar', label: {normal: {show: false, position: 'inside', color: 'white'}},itemStyle: {borderRadius: [4, 4, 0, 0], color: '#d6b160', shadowColor: 'rgba(0, 0, 0, 0.5)', shadowBlur: 2},  //Series设置
        ...
        data: [0, 0, 6, 25, 44, 33, 10, 6, 5, 10, 15, 30, 10, 7, 0], },]}
  ...
  var weekend= {              
    ...
    series: [{name: 'Pit', type: 'bar', label: {normal: {show: false, position: 'inside', color: 'white'}},itemStyle: {borderRadius: [4, 4, 0, 0], color: '#9E7E38', shadowColor: 'rgba(0, 0, 0, 0.5)', shadowBlur: 2},  //Series设置
        // 👇 数据录入处 ！！暂定以10为scale进行模拟, 数据源: Google
        data: [0, 0, 5, 15, 30, 25, 7, 3, 2, 6, 7, 25, 10, 3, 0], },]}
...
Page({
    data: {
        ...
        timeTable:[{realTimeTable:'Mon: 11:00 - 21:00'},{realTimeTable:'Tue: 11:00 - 21:00'},{realTimeTable:'Wed: 11:00 - 21:00'},{realTimeTable:'Thu: 11:00 - 21:00'},{realTimeTable:'Fri: 11:00 - 21:00'},{realTimeTable:'Sat: 16:00 - 21:00'},{realTimeTable:'Sun: 16:00 - 21:00'}],
      },
    ...
    onLoad: function (options) { 
      ...
      wx.cloud.database().collection('UpDown').doc('Shorty').get().then(res=>{
       console.log("sb查询成功",res);
       this.setData({
         RST: res.data.ItemList // 所有items全量信息
       })
       }).catch(err=>{
     ...
     // **************** 评论功能所需onLoad **************//
     // 以下是CommentList函数
     wx.cloud.database().collection("comments").doc('Shorty').get()
     .then(res=>{
  ...
  remark(e){
    ...
    wx.cloud.database().collection('comments').doc('Shorty')
    .update({
    data:{
  ...
  delete: function(e) {
    ...
    // 同步云端删除
    db.collection('comments').doc('Shorty').update({
      data: {
        commentList: list
      }
    })
  ...
  isYourComments: function (e) {
    ...
    wx.cloud.database().collection('comments').doc('Shorty').get().then(res=>{
      this.setData({
