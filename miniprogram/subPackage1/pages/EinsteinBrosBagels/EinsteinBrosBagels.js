// pages/EinsteinBrosBagels/EinsteinBrosBagels.js
...
const EB = db.collection('UpDown').doc('EinsteinBagels')
...
function initChart(canvas, width, height, dpr) {  
  ...
  var weekdays= {  
    ...
    series: [{name: 'Pit', type: 'bar', label: {normal: {show: false, position: 'inside', color: 'white'}},itemStyle: {borderRadius: [4, 4, 0, 0], color: '#d6b160', shadowColor: 'rgba(0, 0, 0, 0.5)', shadowBlur: 2},  //Series设置
        ...
        data: [2, 3, 5, 25, 45, 30, 15, 14, 14, 10, 15, 20, 10, 7, 5], },]}
  ...
  var weekend= {   
    ...
    series: [{name: 'Pit', type: 'bar', label: {normal: {show: false, position: 'inside', color: 'white'}},itemStyle: {borderRadius: [4, 4, 0, 0], color: '#d6b160', shadowColor: 'rgba(0, 0, 0, 0.5)', shadowBlur: 2},  //Series设置
        // 👇 数据录入处 ！！暂定以10为scale进行模拟, 数据源: Google
        data: [0, 0, 0, 15, 30, 25, 10, 4, 4, 7, 13, 17, 14, 2, 1], },]}
...
Page({
    data: {
      ...
      REB:[],
        ...
        timeTable:[{realTimeTable:'Mon: 7:00 - 18:00'},{realTimeTable:'Tue: 7:00 - 18:00'},{realTimeTable:'Wed: 7:00 - 18:00'},{realTimeTable:'Thu: 7:00 - 18:00'},{realTimeTable:'Fri: 7:00 - 16:00'},{realTimeTable:'Sat: closed'},{realTimeTable:'Sun: 11:00 - 17:00'}],
    },
...
// **************** 评论功能所需onLoad **************//
      ...
      wx.cloud.database().collection('UpDown').doc('EinsteinBagels').get().then(res=>{
        console.log("EB查询成功",res);
        this.setData({
          REB: res.data.ItemList // 所有items全量信息
        })
        }).catch(err=>{
      ...
      // 初始页面加载CommentList
      wx.cloud.database().collection("comments").doc('EinsteinBrosBagels').get()
      .then(res=>{
...
// ************** 评论功能所需onLoad结束*************//
  ...
  zan: function (item_id) {
    ...
    remark(e){
      ...
      wx.cloud.database().collection('comments').doc('EinsteinBrosBagels')
      .update({
      data:{
    ...
    delete: function(e) {
      ...
      // 同步云端删除
      db.collection('comments').doc('EinsteinBrosBagels').update({
        data: {
          commentList: list
        }
      })
    ...
    isYourComments: function (e) {
      ...
      wx.cloud.database().collection('comments').doc('EinsteinBrosBagels').get().then(res=>{
        this.setData({
