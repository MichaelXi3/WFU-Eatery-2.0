...
const _ = db.command
const VJ = db.collection('UpDown').doc('VillageJuice')
...
function initChart(canvas, width, height, dpr) {  
  ...
  var weekdays= {    
    ...
    series: [{name: 'Pit', type: 'bar', label: {normal: {show: false, position: 'inside', color: 'white'}},itemStyle: {borderRadius: [4, 4, 0, 0], color: '#d6b160', shadowColor: 'rgba(0, 0, 0, 0.5)', shadowBlur: 2},  //Series设置
        ...
        data: [0, 0, 0, 5, 7, 8, 8, 6, 7, 7, 4, 3, 4, 0, 0], },]}
  ...
  var weekend= {    
    ...
    series: [{name: 'Pit', type: 'bar', label: {normal: {show: false, position: 'inside', color: 'white'}},itemStyle: {borderRadius: [4, 4, 0, 0], color: '#d6b160', shadowColor: 'rgba(0, 0, 0, 0.5)', shadowBlur: 2},  //Series设置
        // 👇 数据录入处 ！！暂定以10为scale进行模拟, 数据源: Google
        data: [0, 0, 5, 8, 6, 5, 5, 4, 5, 5, 3, 2, 3, 0, 0], },]}
...
Page({
    data: {
        ...
        timeTable:[{realTimeTable:'Mon: 11:00 - 20:00'},{realTimeTable:'Tue: 11:00 - 20:00'},{realTimeTable:'Wed: 11:00 - 20:00'},{realTimeTable:'Thu: 11:00 - 20:00'},{realTimeTable:'Fri: 11:00 - 20:00'},{realTimeTable:'Sat: 11:00 - 20:00'},{realTimeTable:'Sun: 11:00 - 20:00'}],
      },
    ...
    onLoad: function (options) { 
      ...
      wx.cloud.database().collection('UpDown').doc('VillageJuice').get().then(res=>{
       console.log("VJ查询成功",res);
       this.setData({
         RVJ: res.data.ItemList // 所有items全量信息
       })
       }).catch(err=>{
     ...
     // **************** 评论功能所需onLoad **************//
     // 以下是CommentList函数
     wx.cloud.database().collection("comments").doc('VillageJuice').get()
     .then(res=>{
    ...
    remark(e){
      ...
      wx.cloud.database().collection('comments').doc('VillageJuice')
      .update({
        data:{
    ...
    delete: function(e) {
      ...
      // 同步云端删除
      db.collection('comments').doc('VillageJuice').update({
        data: {
          commentList: list
        }
      })
    ...
    isYourComments: function (e) {
      ...
      wx.cloud.database().collection('comments').doc('VillageJuice').get().then(res=>{
        this.setData({
