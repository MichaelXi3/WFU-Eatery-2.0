...
const _ = db.command
const Market = db.collection('UpDown').doc('336Market')
...
function initChart(canvas, width, height, dpr) {  
  ...
  var weekdays= {    
    ...
    series: [{name: 'Pit', type: 'bar', label: {normal: {show: false, position: 'inside', color: 'white'}},itemStyle: {borderRadius: [4, 4, 0, 0], color: '#d6b160', shadowColor: 'rgba(0, 0, 0, 0.5)', shadowBlur: 2},  //Series设置
        ...
        data: [5, 10, 12, 13, 20, 13, 8, 5, 5, 7, 11, 10, 5, 4, 3], },]}
  ...
  var weekend= {              
    ...
    series: [{name: 'Pit', type: 'bar', label: {normal: {show: false, position: 'inside', color: 'white'}},itemStyle: {borderRadius: [4, 4, 0, 0], color: '#9E7E38', shadowColor: 'rgba(0, 0, 0, 0.5)', shadowBlur: 2},  //Series设置
        // 👇 数据录入处 ！！暂定以10为scale进行模拟, 数据源: Google
        data: [0, 0, 0, 4, 6, 4, 2, 2, 2, 5, 8, 8, 7, 2, 2], },]}
...
Page({
    ...
    onLoad: function (options) { 
      ...
      wx.cloud.database().collection('UpDown').doc('336Market').get().then(res=>{
       console.log("VJ查询成功",res);
       this.setData({
         RMar: res.data.ItemList // 所有items全量信息
       })
       }).catch(err=>{
     ...
     // **************** 评论功能所需onLoad **************//
     // 以下是CommentList函数
     wx.cloud.database().collection("comments").doc('336Market').get()
     .then(res=>{
  ...
  remark(e){
    ...
    wx.cloud.database().collection('comments').doc('336Market')
    .update({
    data:{
  ...
  delete: function(e) {
    ...
    // 同步云端删除
    db.collection('comments').doc('336Market').update({
      data: {
        commentList: list
      }
    })
  ...
  isYourComments: function (e) {
    ...
    wx.cloud.database().collection('comments').doc('336Market').get().then(res=>{
      this.setData({
